...
def surrounding_box(box1, box2):
    ...
    small = [min(box1_min[0], box2_min[0]),
             min(box1_min[1], box2_min[1]),
             min(box1_min[2], box2_min[2])]
    big = [max(box1_max[0], box2_max[0]),
...
def sort_obj_list(obj_list):
    ...
    min_center = [min([center[0] for center in centers]),
                  min([center[1] for center in centers]),
                  min([center[2] for center in centers])]
    max_center = [max([center[0] for center in centers]),
...
class BVHNode:
    ...
    def __init__(self, object_list, parent):
        ...
        if span == 1:
            ...
            self.box_min, self.box_max = surrounding_box(self.left.bounding_box,
...
class BVH:
    ...
    def build(self, obj_list):
        ...
        def save_bvh(node):
            ...
            self.bvh_left_id[id] = node.left.id if node.left is not None else -1
            self.bvh_right_id[id] = node.right.id if node.right is not None else -1
            self.bvh_next_id[id] = node.next.id if node.next is not None else -1
    ...
    @ti.func
    def hit_aabb(self, bvh_id, ray_origin, ray_direction,
                 t_min, t_max):
        intersect = 1
