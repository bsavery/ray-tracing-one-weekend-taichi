...
class Metal(_material):
    ...
    def scatter(in_direction, p, n, color, roughness):
        out_direction = reflect(in_direction.normalized(), n) + roughness * random_in_unit_sphere()
        attenuation = color
...
class Materials:
    def __init__(self, n):
        ...
        ti.root.dense(ti.i, n).place(self.roughness, self.colors, self.mat_index, self.ior)
    ...
    def scatter(self, i, ray_direction, p, n, front_facing):
        ...
        if mat_index == 0:
            reflected, out_origin, out_direction, attenuation = Lambert.scatter(ray_direction, p, n, color)
        elif mat_index == 1:
            reflected, out_origin, out_direction, attenuation = Metal.scatter(ray_direction, p, n, color, roughness)
        else:
            reflected, out_origin, out_direction, attenuation = Dielectric.scatter(ray_direction, p, n, color, ior, front_facing)
